...
class CORSRequestHandler (SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if (datas == 'opt_OpenDB'):
            print 1
            status = os.system('./auto_config.sh stream on')
        elif (datas == 'opt_CloseDB'):
            print 2
            status = os.system('./auto_config.sh stream off')
        elif (datas == 'opt_EnableTSN'):
            print 3
            status = os.system('./auto_config.sh tsn on')
        elif (datas == 'opt_DisableTSN'):
            print 4
            status = os.system('./auto_config.sh tsn off')
        else:
            print 5
            print 'error input!'
